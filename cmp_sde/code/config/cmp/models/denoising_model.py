...
class DenoisingModel(BaseModel):
    ...
    def feed_data(self, state, LQ,SF_B,SF_W,SF_G, GT=None):
        self.state = state.to(self.device)    # noisy_state
        self.condition = LQ.to(self.device) #  torch.cat( [LQ.to(self.device), SF.to(self.device) ],1 ) # LQ+SF
        self.s =  SF_G.to(self.device)
        self.sb =  SF_B.to(self.device)
        self.sw =  SF_W.to(self.device)
        
        if GT is not None:
            self.state_0 = GT.to(self.device)  # GT

    def optimize_parameters(self, step, timesteps, sde=None):
        sde.set_mu(self.condition,self.s,self.sb,self.sw)
    ...
    def get_current_visuals(self,current_step=0, need_GT=True):
        out_dict = OrderedDict()
        out_dict["Input"] = self.condition.detach()[0].float().cpu()
        out_dict["S"] = self.s.detach()[0].float().cpu()
