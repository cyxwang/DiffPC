...
for test_loader in test_loaders:
    test_set_name = test_loader.dataset.opt["name"] +  test_loader.dataset.opt["dataname"][0][0].replace('/','_')  # path opt['']
    logger.info("\nTesting [{:s}]...".format(test_set_name))
    test_start_time = time.time()
    dataset_dir = os.path.join(opt["path"]["results_root"], test_set_name)
    util.mkdir(dataset_dir)
    util.mkdir( os.path.join(dataset_dir,'sr'))
    util.mkdir( os.path.join(dataset_dir,'hr'))
    util.mkdir( os.path.join(dataset_dir,'lr'))
    ...
    for i, test_data in enumerate(test_loader):
        ...
        single_img_psnr_y = []
        single_img_ssim_y = []
        need_GT = True # False if test_loader.dataset.opt["dataroot_GT"] is None else True
        img_path = test_data["GT_path"][0] if need_GT else test_data["LQ_path"][0]
        img_name = os.path.splitext(os.path.basename(img_path))[0]
        

        #### input dataset_LQ
        LQ, SF_B,SF_W,SF_G, GT = test_data["LQ"],test_data["SF_B"],test_data["SF_W"],test_data["SF_G"], test_data["GT"]
        # print("SF_B_path:",test_data['SF_B_path'])
        # print("SF_W_path:",test_data['SF_W_path'])
        # print("SF_G_path:",test_data['SF_G_path'])
        ...
        save_img_path = os.path.join(dataset_dir, 'sr',img_name + ".png")
        util.save_img(output, save_img_path)

        # remove it if you only want to save output images
        LQ_img_path = os.path.join(dataset_dir, 'lr', img_name + ".png")
        GT_img_path = os.path.join(dataset_dir, 'hr', img_name + ".png")
        ...
        if need_GT:
            ...
            crop_border = 0 #  opt["crop_border"] if opt["crop_border"] else scale
            if crop_border == 0:
