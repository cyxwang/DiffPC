...
class IRSDE(SDE):
    ...
    '''
    def __init__(self, max_sigma, T=100, schedule='cosine', eps=0.01, beta_start=0.0001,beta_end=0.05,s = 0.0008, device=None):
        super().__init__(T, device)
        self.max_sigma = max_sigma / 255 if max_sigma >= 1 else max_sigma
        self._initialize(self.max_sigma, T, schedule, eps,beta_start,beta_end,s)

    def _initialize(self, max_sigma, T, schedule, eps=0.01,beta_start=0.0001,beta_end=0.05,s = 0.0008):
        ...
        def linear_theta_schedule(timesteps,beta_start=0.0001,beta_end=0.05):
            """
            linear schedule
            """
            ...
            beta_start = scale *  beta_start  # 0.001 #
            beta_end = scale * beta_end # 0.05
            return torch.linspace(beta_start, beta_end, timesteps, dtype=torch.float32)

        def cosine_theta_schedule(timesteps, s = 0.0008): # 0.02
            """
            cosine schedule
            """
        ...
        if schedule == 'cosine':
            thetas = cosine_theta_schedule(T,s)
        elif schedule == 'linear':
            thetas = linear_theta_schedule(T,beta_start,beta_end)
    ...
    def score_fn(self, x, t, **kwargs):
        ...
        noise = self.model(x, self.mu,self.s,self.sb,self.sw, t, **kwargs)
        return self.get_score_from_noise(noise, t)

    def noise_fn(self,x,t, **kwargs):
        # need to pre-set mu and score_model
        return self.model(x, self.mu,self.s,self.sb,self.sw, t, **kwargs)
    ...
    def generate_random_states(self, x0, mu,s,sb,sw):
